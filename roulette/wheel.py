...
class Wheel(object) :
  ...
  def __init__(self, rng=None):
    ...
    self.all_outcomes = [] #acting as a factory class to hold all outcomes
  ...
  def getOutcome(self, name):
    for oc in self.all_outcomes:
      if name == oc.name:
        return oc
#    return set( [ oc for oc in self.all_outcomes if name.lower() in oc.name.lower() ] ) #why are we returning a set?
